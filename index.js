...
app.post('/api/users/:_id/exercises', function(req, res) {
  ...
  User.findById(user_id, (err, data) => {
    ...
    exercise.save((err, data) => {
      if (err) return console.error(err);
      res.json({_id: data._id, username: username, date: data.date.toDateString() , duration: data.duration, description: data.description});
    });
  });
});
  ...
  User.findById(user_id, (err, data) => {
    ...
    if(req.query.from || req.query.to) {
      ...
      Exercise.find({username: username, date: dateObj}, (err, data) => {
        if (err) return console.error(err);
        res.json({_id: data._id, username: username, count: data.length, log: data});
      });
    }
    // /api/users/:_id/logs?[limit]
    else if(req.query.limit) {
      Exercise.find({username: username}).limit(parseInt(req.query.limit)).exec((err, data) => {
        if (err) return console.error(err);
        res.json({_id: data._id, username: username, count: data.length, log: data});
      });
    }
